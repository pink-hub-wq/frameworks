...
function updateShopInfoNumber(obj) {
    ...
    window.location.href = "shops_updateShopInfoNumber?shopId=" + id + "&number=" + number;
}
function deleteShopInfo(obj) {
	var id = $(obj).closest('tr').find('.shop-id').text();
    window.location.href = "shops_deleteShopInfo&shopId=" + id;
}
</script>
</head>
<body>
	<table border="1px">
		<tr>
			...
			<td colspan="2"><a href="shops_query">返回继续购物</a></td>
		</tr>
		<tr>
			<td>编号</td>
			<td>商品名称</td>
		...
		<c:forEach var="cart" items="${cart}">
			<tr>
				...
				<td><a href="shops_delete?shopId=${cart.id}">退回此商品</a></td>
			</tr>
			 <c:set var="totalAmount" value="${totalAmount + cart.totalPrice}" />
		</c:forEach>
		<tr>
			<td colspan="4"><a href="shops_clearCart">清空购物车</a></td>
			<td colspan="3">购物车总金额：${totalAmount }</td>
		</tr>
		<tr>
            <td colspan="7">
                <form action="order_creatOrder" method="post">
