...
public class MyBeanFactory {
	public static UserService createService() {
		...
		/*
		 * 3 代理类：将目标类（切入点）和 切面类（通知） 结合 --> 切面 Proxy.newProxyInstance
		 *
		 * 参数1：loader ，类加载器，动态代理类 运行时创建，任何类都需要类加载器将其 加载到内存。
		 *
		 ...
		 *
		 * 方式1：目标类实例.getClass().getInterfaces() ;注意：只能获得自己接 口，不能获得父元素接口
		 *
